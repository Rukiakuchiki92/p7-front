...
def call_prediction_api(client_id):
    ...
    if response.status_code == 200:
        ...
        st.error("Erreur lors de l'appel de l'API de prédiction : " + response.text)
        return None
...
def call_client_info_api(client_id):
    ...
    if response.status_code == 200:
        ...
        st.error("Erreur lors de l'appel de l'API d'informations client : " + response.text)
        return None

# Fonction pour appeler l'API et récupérer les valeurs SHAP
def call_shap_api(client_id):
    endpoint = f"Shap/{client_id}"
    url = URI + endpoint
    response = requests.get(url)
    
    if response.status_code == 200:
        data = response.json()
        return data.get('shap_values', None)
    else:
        st.error("Erreur lors de l'appel de l'API SHAP : " + response.text)
        return None
        ...
        if st.button("Prédire"):
            ...
            if prediction_data:
                ...
                if client_info_data:
                    ...
                    # Liste des colonnes de niveau d'éducation
                    ...
                    # Parcourir chaque colonne de niveau d'éducation
                    for colonne in colonnes_ecole:
                        # Récupérer la valeur pour cette colonne
                        valeur_statut_ecole = client_info_data.get(colonne, None)
                        
                        # Appliquer le mapping pour obtenir le libellé
                        label_ecole = ecole_mapping.get(colonne, "N/A")
    
                        # Afficher le libellé pour cette colonne
                        st.write(f"{label_ecole} :", "Oui" if valeur_statut_ecole == 1 else "Non")
    
                    # Mapping pour le type de revenu
                    type_revenu_mapping = {
                    ...
                    # Liste des colonnes de type de revenu
                    colonnes_type_revenu = ["TYPE_REVENUS_Maternity leave", "TYPE_REVENUS_Pensioner", "TYPE_REVENUS_State servant", "TYPE_REVENUS_Unemployed", "TYPE_REVENUS_Working"]
                    
                    # Parcourir chaque colonne de type de revenu
                ...
                # Appel de l'API pour récupérer les valeurs SHAP
                shap_values = call_shap_api(id_client)
                
                if shap_values:
                    st.subheader("Valeurs SHAP :")
                    # Convertir les valeurs SHAP en DataFrame pour une visualisation
                    try:
                        shap_values_df = pd.DataFrame([shap_values])
                        
                        # Assurez-vous que shap_values est un tableau NumPy
                        shap_values_np = np.array(list(shap_values.values()))
    
                        # Utilisation de SHAP pour créer le graphique
                        st.set_option('deprecation.showPyplotGlobalUse', False)
                        shap.summary_plot(shap_values_np, feature_names=shap_values_df.columns, plot_type="bar")
                        st.pyplot(bbox_inches='tight')
                    except Exception as e:
                        st.error(f"Erreur lors de la visualisation des valeurs SHAP : {e}")
                        st.error(traceback.format_exc())
                else:
                    st.write("Erreur lors de la récupération des données SHAP. Veuillez réessayer plus tard.")
